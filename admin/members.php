<?php 
    require 'server/connect.php';
     ...
     if(!isset($_SESSION["orgUserName"]) && empty($_SESSION["orgUserName"])){ 
        ...
        $getUsers = mysqli_query($conn,"SELECT * FROM users") or die(mysqli_error($conn)); 
        $usersArray = array();
                  ...
                  <div class="table-responsive">
                    <div id="recent-purchases-listing_wrapper" class="dataTables_wrapper container-fluid dt-bootstrap4 no-footer"><div class="row"><div class="col-sm-12 col-md-6"></div><div class="col-sm-12 col-md-6"></div></div><div class="row"><div class="col-sm-12"><table id="recent-purchases-listing" class="table dataTable no-footer" role="grid">
                      <thead>
                        ...
                        <th class="sorting_asc" tabindex="0" aria-controls="recent-purchases-listing" rowspan="1" colspan="1" aria-sort="ascending" aria-label="Name: activate to sort column descending" style="width: 122px;">Organisation Name</th>
                      ...
                      <tbody>
                        ...
                        <tr role="row" class="odd<?php if($singleUser['memberStatus']=='disabled'){echo ' table-danger'; } ?>">
                            ...
                            <?php if($singleUser['organisationID']!=null){ 
                                $organisationID = $singleUser['organisationID'];
                                $searchOrg = mysqli_query($conn,"SELECT * FROM organization WHERE id='$organisationID'") or die(mysqli_error($conn)); 
                                $searchOrg = mysqli_fetch_assoc($searchOrg);
                            ?> 
                                <td><?php echo $searchOrg['organisationName'] ?></td>     
                            <?php }else{ ?>
