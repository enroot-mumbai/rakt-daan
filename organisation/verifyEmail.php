<?php 
 require'server/connect.php';
        ...
        if(isset($_GET['email']) && !empty($_GET['email']) AND isset($_GET['hash']) && !empty($_GET['hash'])){
            ...
            $query = "select * from $table where organiserEmail= $email AND organisationEmailHash='$hashnew' AND organiserVerified=0";
            ...
            if($match > 0){
                mysqli_query($conn,"UPDATE $table SET organiserVerified='1' WHERE organiserEmail=$email AND organisationEmailHash='$hashnew' AND organiserVerified=0") or die(mysqli_error($conn));
